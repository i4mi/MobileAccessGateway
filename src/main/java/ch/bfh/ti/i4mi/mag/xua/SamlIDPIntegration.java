...
public class SamlIDPIntegration extends WebSecurityConfigurerAdapter implements InitializingBean, DisposableBean {
    ...
    public OAuth2TokenEncryptionService oAuth2TokenEncryptionService() throws Exception {
        ...
        final KeyStore.Entry entry = keyStore.getEntry(this.cryptKeyAlias,
                new KeyStore.PasswordProtection(this.cryptKeystorePass.toCharArray()));
        if (entry instanceof KeyStore.SecretKeyEntry) {
            return new OAuth2TokenEncryptionService(((KeyStore.SecretKeyEntry) entry).getSecretKey());
        } else {
            throw new IllegalArgumentException("No SecretKey found for alias: " + this.cryptKeyAlias);
        }
    }
}
